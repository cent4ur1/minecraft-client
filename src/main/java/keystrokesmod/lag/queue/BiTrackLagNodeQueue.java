...
public final class BiTrackLagNodeQueue {
    ...
    public void releaseNextPacket(final @NotNull EnumLagDirection direction) {
        switch (direction) {
            case OUTBOUND:
                outgoingState.releaseNextPacket();
                break;
            case INBOUND:
                incomingState.releaseNextPacket();
                break;
        }
    }
    ...
    private static final class TrackState {
        ...
        private synchronized void releaseNextPacket() {
            for (int i = 0; i < track.size(); i++) {
                AbstractLagNode node = track.get(i);
                if (!(node instanceof PacketLagNode)) {
                    continue;
                }

                PacketLagNode packetLagNode = (PacketLagNode) node;
                track.remove(i);
                packetLagNode.goThrough(fastTrackProvider);
                return;
            }
        }

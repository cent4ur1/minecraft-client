...
public class ItemESP extends Module {
    private final ButtonSetting disableDiamond;
    private final ButtonSetting disableEmerald;
    private final ButtonSetting disableGold;
    private final ButtonSetting disableIron;

    public ItemESP() {
        super("ItemESP", category.render);
        this.liteModule = true;
        this.registerSetting(disableDiamond = new ButtonSetting("Disable diamond", false));
        this.registerSetting(disableEmerald = new ButtonSetting("Disable emerald", false));
        this.registerSetting(disableGold = new ButtonSetting("Disable gold", false));
        this.registerSetting(disableIron = new ButtonSetting("Disable iron", false));
    }
    ...
    public void onRenderWorldLast(RenderWorldLastEvent e) {
        ...
        if (!itemsMap.isEmpty()) {
            ...
            for (Map.Entry<Item, ArrayList<EntityItem>> entry : itemsMap.entrySet()) {
                ...
                int boxColor;
                int textColor;
                if (item == Items.iron_ingot && !disableIron.isToggled()) {
                    textColor = (boxColor = -1);
                }
                else if (item == Items.gold_ingot && !disableGold.isToggled()) {
                    boxColor = -331703;
                    textColor = -152;
                }
                else if (item == Items.diamond && !disableDiamond.isToggled()) {
                    boxColor = -10362113;
                    textColor = -7667713;
                }
                else {
                    if (item != Items.emerald || disableEmerald.isToggled()) {
                        continue;
                    }
                    boxColor = -15216030;
                    textColor = -14614644;

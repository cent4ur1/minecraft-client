...
public class Indicators extends Module {
    ...
    private static final double FIREBALL_ACCELERATION = 0.1D;
    ...
    private final Map<Integer, Vec3> fireballAccelerationOverrides = new ConcurrentHashMap<>();
    ...
    public void onDisable() {
        ...
        fireballAccelerationOverrides.clear();
    }
    ...
    public void onReceivePacket(ReceivePacketEvent event) {
        ...
        if (motionLength > 0.0D) {
            // Deflections sync motion to the client, but not EntityFireball's new acceleration vector.
            double accelerationScale = FIREBALL_ACCELERATION / motionLength;
            fireballAccelerationOverrides.put(packet.getEntityID(), new Vec3(
                    motionX * accelerationScale,
                    motionY * accelerationScale,
                    motionZ * accelerationScale
            ));
        }
    }
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        Set<Integer> loadedFireballs = new HashSet<>();
        entitiesToRender.clear();
        ...
        for (Entity en : mc.theWorld.loadedEntityList) {
            ...
            if (en instanceof EntityLargeFireball) {
                loadedFireballs.add(en.getEntityId());
            }
        ...
        for (Integer entityId : fireballAccelerationOverrides.keySet()) {
            if (!loadedFireballs.contains(entityId)) {
                fireballAccelerationOverrides.remove(entityId);
            }
        }
    }
    ...
    private void renderFireballTrajectory(EntityLargeFireball fireball, float partialTicks) {
        Vec3 accelerationOverride = fireballAccelerationOverrides.get(fireball.getEntityId());
        FireballSImulator.Result result = FireballSImulator.simulate(fireball, accelerationOverride);

...
public class KillAura extends Module {
        ...
        if (rotationMode.getInput() == 0) {
            ...
            if (targetDistance <= aimRangeVal) {
                ...
                float[] rot = RotationHelper.get().getRotationsToTarget(
                        ...
                        useBackup,
                        aimRangeVal,
                        aimThroughBlocks.isToggled(),
                        aimThroughEntities.isToggled(),
                        true
                );
    ...
    public void onPrePlayerInteract(PrePlayerInteractEvent e) {
        ...
        if (notUsingItem.isToggled() && mc.thePlayer.isUsingItem()) {
            return;
        }
    ...
    private Candidate getCandidateTarget(Entity entity, double maxRange, float fovValue) {
        ...
        boolean targetableUnicorn = entity instanceof EntityHorse && BedWars.shouldTargetUnicorn();
        ...
        } else if (targetableUnicorn) {
            if (((EntityHorse) entity).deathTime != 0) {
                return null;
            }
    ...
    private KillAuraTarget buildKillAuraTarget(EntityLivingBase entity, double distanceToBoundingBox, double maxRange) {
        boolean targetableUnicorn = entity instanceof EntityHorse && BedWars.shouldTargetUnicorn();
        if (entity instanceof EntityCreature && !targetableUnicorn && attackMobs.isToggled() && !isHostile((EntityCreature) entity)) {
            return null;
        }

...
public class Autoblock extends Module {
    ...
    private final ButtonSetting forceAttack;
    private int pendingForceAttacks;
    private boolean forceAttackUnblocked;
    ...
    public Autoblock() {
        ...
        this.registerSetting(forceAttack = new ButtonSetting("Force attack", false));
    ...
    public void onPrePlayerInteract(PrePlayerInteractEvent e) {
        ...
        if (!forceAttack.isToggled() || !mc.inGameHasFocus) {
            pendingForceAttacks = 0;
        }
        if (pendingForceAttacks > 0) {
            forceAttackUnblocked = true;
            releaseLag();
            stopBlocking(true);
            manualBlock = false;
            return;
    ...
    public void onForceAttack(PrePlayerInteractEvent e) {
        if (!forceAttack.isToggled() || !Utils.nullCheck() || mc.thePlayer.isDead
                || mc.currentScreen != null || !mc.inGameHasFocus || !Utils.holdingSword()) return;
        if (ModuleManager.bedAura != null && ModuleManager.bedAura.isActivelyMining()) return;
        if (allowingAlwaysInteraction) {
            pendingForceAttacks = 0;
            return;
        ...
        if (pendingForceAttacks > 0 && !mc.thePlayer.isUsingItem()) {
            for (int i = 0; i < pendingForceAttacks; i++) {
                KeyBinding.onTick(attackKey.getKeyCode());
            }
            pendingForceAttacks = 0;
            return;
        }
        if (!isBlocking && !isLagging && !mc.thePlayer.isBlocking() && pendingForceAttacks == 0) return;
        boolean attackPressed = attackKey.isPressed();
        if (!attackPressed && pendingForceAttacks == 0) return;
    ...
    private boolean shouldBlockVanillaUse() {
        return isEnabled() && (isLagging || forceAttackUnblocked) && Utils.nullCheck() && Utils.holdingSword() && mc.currentScreen == null;
    }

    private void startBlocking(int currentTick) {
        if (forceAttackUnblocked || !Utils.holdingSword() || isCooldownActive()) return;

...
public class BridgeAssist extends Module {
    ...
    public void onSlotScroll(PreSlotScrollEvent e) {
        if (disableHotbarScrolling.isToggled() && isAssistActive()) {
            e.setCanceled(true);
        }
    }

    private boolean isAssistActive() {
        if (!isEnabled() || !Utils.nullCheck() || mc.currentScreen != null || mc.thePlayer.capabilities.isFlying) {
            return false;
        }
        ...
        if (manualSneak && !sneakKeyPressed.isToggled()) {
            return false;
        }
        if (sneakKeyPressed.isToggled() && (!manualSneak || forward == 0 && strafe == 0)) {
            return false;
        }
        if (notMovingForward.isToggled() && forward > 0) {
            return false;
        }
        if (lookingDown.isToggled() && mc.thePlayer.rotationPitch < 70) {
            return false;
    ...
    private double computeEdgeOffset(AxisAlignedBB simBox) {
        AxisAlignedBB groundCheck = new AxisAlignedBB(
                simBox.minX, simBox.minY - 0.51, simBox.minZ,

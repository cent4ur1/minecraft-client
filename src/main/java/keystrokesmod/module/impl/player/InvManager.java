...
public class InvManager extends Module {
    ...
    private final SliderSetting autoSortMode;
    private final SliderSetting normalSortDelay;
    ...
    private final SliderSetting speedSlot;
    ...
    public InvManager() {
        ...
        this.registerSetting(autoSortMode = new SliderSetting("Auto sort mode", true, AUTO_SORT_NORMAL, new String[]{ "Normal", "Custom" }));
        this.registerSetting(normalSortDelay = new SliderSetting("Auto sort", " tick", 3.0, 1.0, 20.0, 1.0, "Normal auto sort.Auto sort"));
        this.registerSetting(swordSlot = new SliderSetting("Sword slot", true, -1, 1, 9, 1, "Normal auto sort.Sword slot"));
        this.registerSetting(blocksSlot = new SliderSetting("Blocks slot", true, -1, 1, 9, 1, "Normal auto sort.Blocks slot"));
        this.registerSetting(goldenAppleSlot = new SliderSetting("Golden apple slot", true, -1, 1, 9, 1, "Normal auto sort.Golden apple slot"));
        this.registerSetting(projectileSlot = new SliderSetting("Projectile slot", true, -1, 1, 9, 1, "Normal auto sort.Projectile slot"));
        this.registerSetting(speedSlot = new SliderSetting("Speed potion slot", true, -1, 1, 9, 1, "Normal auto sort.Speed potion slot"));
        this.registerSetting(pearlSlot = new SliderSetting("Pearl slot", true, -1, 1, 9, 1, "Normal auto sort.Pearl slot"));
        this.registerSetting(targetCPS = new SliderSetting("Target CPS", 10.0, 1.0, 20.0, 0.5, "Auto sort.Target CPS"));
        this.registerSetting(disableWhenComplete = new ButtonSetting("Disable when complete", false, "Auto sort.Disable when complete"));
        this.registerSetting(stackItems = new ButtonSetting("Stack items", false, "Auto sort.Stack items"));
        this.registerSetting(items = new InventoryItemListSetting("Items", "Auto sort.Items"));
        this.closetModule = true;
    }

    @Override
    public void guiUpdate() {
        boolean normalMode = isAutoSortMode(AUTO_SORT_NORMAL);
        normalSortDelay.setVisible(normalMode, this);
        swordSlot.setVisible(normalMode, this);
        blocksSlot.setVisible(normalMode, this);
        goldenAppleSlot.setVisible(normalMode, this);
        projectileSlot.setVisible(normalMode, this);
        speedSlot.setVisible(normalMode, this);
        pearlSlot.setVisible(normalMode, this);

        boolean customMode = isAutoSortMode(AUTO_SORT_CUSTOM);
        targetCPS.setVisible(customMode, this);
        disableWhenComplete.setVisible(customMode, this);
        stackItems.setVisible(customMode, this);
        items.setVisible(customMode, this);
    }

    @Override
    public void onEnable() {
        migrateLegacyAutoSortSetting();
        ticks = 0L;
        nextDelay = 0L;
        closeGui = false;
    ...
    public void guiButtonToggled(ButtonSetting buttonSetting) {
        if (!isAutoSortMode(AUTO_SORT_CUSTOM) || buttonSetting != disableWhenComplete || !isManagedInventoryOpen()) {
            return;
        }
    ...
    @SubscribeEvent
    public void onProfileLoad(PostProfileLoadEvent event) {
        migrateLegacyAutoSortSetting();
    }

    private void migrateLegacyAutoSortSetting() {
    ...
    private boolean shouldPauseForLobby() {
        return (disableInLobby.isToggled() && Utils.isLobby())
            ...
            && ModuleManager.invmove.isCloseInventoryMode()
    ...
    private void handleInventoryScreen() {
        ...
        if (isAutoSortMode(AUTO_SORT_CUSTOM)) {
            ...
            stopCustomSorter();
            if (isAutoSortMode(AUTO_SORT_NORMAL) && tryNormalAutoSortAction()) {
                return;
            }
        }
    ...
    private boolean isAutoSortMode(int mode) {
        return (int)autoSortMode.getInput() == mode;
    }

    private void stopCustomSorter() {
        if (!sessionOpen && currentAction == null && cursorRecoveryInventoryIndex < 0) {
            return;
        }

        InventorySnapshot snapshot = InventorySnapshot.capture();
        if (ownsCarriedStack() && snapshot.carried != null) {
            recoverCarriedStackImmediately(snapshot, false);
        }
        closeSession();
    }

    private boolean tryNormalAutoSortAction() {
        if (mc.thePlayer.inventory.getItemStack() != null) {
            return false;
        }

        InventoryData data = new InventoryData(mc.thePlayer.inventory, true, false);
        if (fixNormalSwordSlot(data)) {
            return true;
        }
        ...
        for (int inventoryIndex = 0; inventoryIndex < data.size; ++inventoryIndex) {
            ...
            if (item instanceof ItemBlock) {
                ...
                ItemStack currentBlocks = data.inventory.getStackInSlot(targetSlot);
                ...
                if (canStacksMerge(itemStack, currentBlocks)) {
                    if (itemStack.stackSize == itemStack.getMaxStackSize()) {
                        normalSortClick(inventoryIndex, targetSlot, 2);
                        return true;
                    }
                    if (data.emptyWithoutHotbar != 0 || inventoryIndex >= HOTBAR_SIZE) {
                ...
                int targetSlot = configuredHotbarSlot(goldenAppleSlot);
                if (targetSlot < 0 || inventoryIndex == targetSlot) {
                    continue;
                }

                ItemStack currentGoldenApple = data.inventory.getStackInSlot(targetSlot);
                ...
                int targetSlot = configuredHotbarSlot(projectileSlot);
                if (targetSlot < 0 || inventoryIndex == targetSlot) {
                    continue;
                }

                ItemStack currentProjectile = data.inventory.getStackInSlot(targetSlot);
                ...
                else if (currentProjectile.stackSize < currentProjectile.getMaxStackSize()) {
                    if (item == currentProjectile.getItem()) {
                        if (data.emptyWithoutHotbar != 0 || inventoryIndex >= HOTBAR_SIZE) {
                            normalSortClick(inventoryIndex, 0, 1);
                            return true;
                        }
                ...
                int targetSlot = configuredHotbarSlot(speedSlot);
                if (targetSlot >= 0
                    && inventoryIndex != targetSlot
                    && isSpeedPotion(itemStack)
                    && !isSpeedPotion(data.inventory.getStackInSlot(targetSlot))) {
                    normalSortClick(inventoryIndex, targetSlot, 2);
                    return true;
                }
            }
            else if (item instanceof ItemEnderPearl) {
                int targetSlot = configuredHotbarSlot(pearlSlot);
                if (targetSlot < 0 || inventoryIndex == targetSlot) {
                    continue;
                }

                ItemStack currentPearl = data.inventory.getStackInSlot(targetSlot);
    ...
    private boolean fixNormalSwordSlot(InventoryData data) {
        int targetSlot = configuredHotbarSlot(swordSlot);
        if (targetSlot < 0) {
            return false;
        }

        ItemStack targetStack = data.inventory.getStackInSlot(targetSlot);
        double bestDamage = isSword(targetStack) ? Utils.getDamageLevel(targetStack) : -1.0;
        int bestInventoryIndex = -1;
        for (int inventoryIndex = 0; inventoryIndex < data.size; ++inventoryIndex) {
            if (inventoryIndex == targetSlot) {
                continue;
            }

            ItemStack candidate = data.inventory.getStackInSlot(inventoryIndex);
            if (!isSword(candidate)) {
                continue;
            }

            double damage = Utils.getDamageLevel(candidate);
        ...
        normalSortClick(bestInventoryIndex, targetSlot, 2);
        return true;
    }

    private int configuredHotbarSlot(SliderSetting setting) {
        return (int)setting.getInput() - 1;
    }

    private void normalSortClick(int inventoryIndex, int button, int mode) {
        delayedClick(toContainerSlot(inventoryIndex), button, mode, normalSortDelay);
    }
    ...
    private void handleChestScreen() {
        ...
        if (!isAutoSortMode(AUTO_SORT_CUSTOM)) {
            handleNormalChestStealer(chestData, playerData);
            return;
        }
    ...
    private void handleNormalChestStealer(InventoryData chestData, InventoryData playerData) {
        if (playerData.size != playerData.filled) {
            if (stealBestSwordToNormalSlot(chestData, playerData)) {
                return;
            }

            for (int armorIndex = 0; armorIndex < chestData.armorData[0].length; ++armorIndex) {
                if (chestData.armorData[1][armorIndex] == -1) {
                    continue;
                }

                CurrentArmor currentArmor = armorArr[armorIndex];
                if (chestData.armorData[1][armorIndex] > currentArmor.defenceLevel) {
                    normalChestClick(chestData.armorData[0][armorIndex], 0, 1);
                    return;
                }
            }

            for (int chestSlot = 0; chestSlot < chestData.size; ++chestSlot) {
                ItemStack chestStack = chestData.inventory.getStackInSlot(chestSlot);
                if (chestStack == null || isSword(chestStack) || isArmor(chestStack) || shouldSkipChestItem(chestStack, playerData)) {
                    continue;
                }
                ...
                if (item instanceof ItemBlock) {
                    int targetSlot = configuredHotbarSlot(blocksSlot);
                    if (targetSlot < 0) {
                        normalChestClick(chestSlot, 0, 1);
                        return;
                    }
                    ...
                    ItemStack currentGoldenApple = playerData.inventory.getStackInSlot(targetSlot);
                    if (currentGoldenApple == null || !(currentGoldenApple.getItem() instanceof ItemAppleGold)) {
                        normalChestClick(chestSlot, targetSlot, 2);
                        return;
                    }
                    ...
                    ItemStack currentProjectile = playerData.inventory.getStackInSlot(targetSlot);
                    if (currentProjectile == null
                        || (!(currentProjectile.getItem() instanceof ItemSnowball)
                        && !(currentProjectile.getItem() instanceof ItemEgg))) {
                        normalChestClick(chestSlot, targetSlot, 2);
                    ...
                    if (chestStack.stackSize > currentProjectile.stackSize) {
                        if (item == currentProjectile.getItem() && chestStack.stackSize <= chestStack.getMaxStackSize()) {
                            normalChestClick(chestSlot, 0, 1);
                        }
                        else {
                            normalChestClick(chestSlot, targetSlot, 2);
                ...
                else if (item instanceof ItemPotion && isSpeedPotion(chestStack)) {
                    int targetSlot = configuredHotbarSlot(speedSlot);
                    if (targetSlot < 0) {
                        normalChestClick(chestSlot, 0, 1);
                        return;
                    ...
                    if (!isSpeedPotion(playerData.inventory.getStackInSlot(targetSlot))) {
                        normalChestClick(chestSlot, targetSlot, 2);
                        return;
                    }
                }
                    ...
                    ItemStack currentPearl = playerData.inventory.getStackInSlot(targetSlot);
                    if (currentPearl == null || !(currentPearl.getItem() instanceof ItemEnderPearl)) {
                        normalChestClick(chestSlot, targetSlot, 2);
                        return;
                    }
            ...
            for (int chestSlot = 0; chestSlot < chestData.size; ++chestSlot) {
                ItemStack chestStack = chestData.inventory.getStackInSlot(chestSlot);
                if (chestStack == null || shouldSkipChestItem(chestStack, playerData) || !isNormalMergeItem(chestStack)) {
                    continue;
                }

                for (int inventoryIndex = 0; inventoryIndex < playerData.size; ++inventoryIndex) {
                    ItemStack inventoryStack = playerData.inventory.getStackInSlot(inventoryIndex);
                    if (inventoryStack != null
                        && canStacksMerge(chestStack, inventoryStack)
    ...
    private boolean stealBestSwordToNormalSlot(InventoryData chestData, InventoryData playerData) {
        int targetSlot = configuredHotbarSlot(swordSlot);
        if (targetSlot < 0 || chestData.swordData[1] == -1.0) {
            return false;
        }

        ItemStack currentSword = playerData.inventory.getStackInSlot(targetSlot);
        double currentDamage = isSword(currentSword) ? Utils.getDamageLevel(currentSword) : -1.0;
        if (chestData.swordData[1] <= currentDamage) {
            return false;
        }

        normalChestClick((int)chestData.swordData[0], targetSlot, 2);
        return true;
    }

    private boolean isNormalMergeItem(ItemStack stack) {
    ...
    private void normalChestClick(int chestSlot, int button, int mode) {
        delayedClick(chestSlot, button, mode, chestStealer);
    }
    ...
    private int getAssignedHotbarSlot(ItemStack stack) {
        if (!isAutoSortMode(AUTO_SORT_CUSTOM) || stack == null) {
            return -1;
        }
    ...
    private boolean shouldSkipChestItem(ItemStack stack, InventoryData playerData) {
        Item item = stack.getItem();
        if (isAutoSortMode(AUTO_SORT_CUSTOM) && items.matches(stack)) {
            return false;
        }
    ...
    public boolean shouldCancelManualInventoryInput() {
        return isEnabled()
            && isAutoSortMode(AUTO_SORT_CUSTOM)

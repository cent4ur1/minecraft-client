...
public class Jump45 extends Module {
    ...
    private final ButtonSetting movingForward;
    private final ButtonSetting inLiquid;
    ...
    public Jump45() {
        ...
        this.registerSetting(movingForward = new ButtonSetting("Moving forward", true));
        this.registerSetting(inLiquid = new ButtonSetting("In liquid", false));
        this.registerSetting(targeting = new ButtonSetting("Targeting", false));
    }

    @SubscribeEvent(priority = EventPriority.LOWEST)
    public void onJump(JumpEvent e) {
        boolean inEnabledLiquid = inLiquid.isToggled() && isInLiquid();
        if (mc.thePlayer == null || mc.thePlayer.movementInput == null || e.entity != mc.thePlayer || !e.applySprint()
                || !mc.thePlayer.onGround && !inEnabledLiquid
    ...
    private boolean canActivate() {
        boolean hasActivationFilter = holdingBlocks.isToggled()
                || holdingRod.isToggled()
                || holdingThrowable.isToggled()
                || movingForward.isToggled()
                || inLiquid.isToggled();
        if (!hasActivationFilter) {
            return true;
        }

        if (movingForward.isToggled() && !Utils.isBindDown(mc.gameSettings.keyBindForward)) {
            return true;
        }

        if (inLiquid.isToggled() && isInLiquid()) {
            return true;
        }

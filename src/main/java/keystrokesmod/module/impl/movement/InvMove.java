...
public class InvMove extends Module {
    ...
    public void guiUpdate() {
        ...
        delayOpenPacket.setVisible(inventory.getInput() == INVENTORY_MODE_LEGIT, this);
    }

    @Override
    public String getInfo() {
        return INVENTORY_MODES[(int) inventory.getInput()];
    }
    ...
    public void onPreUpdate(PreUpdateEvent e) {
        ...
        boolean applyAllowSettings = shouldApplyAllowSettings();
        restoreRestrictedInput = applyAllowSettings && (!allowSneak.isToggled() || !allowMidair.isToggled() && !mc.thePlayer.onGround);
        boolean sneakRequested = isSneakRequested();
        boolean sprintingAllowed = !applyAllowSettings || allowSprinting.isToggled();
        boolean foodLvlMet = (float)mc.thePlayer.getFoodStats().getFoodLevel() > 6.0F || mc.thePlayer.capabilities.allowFlying; // from mc
        if (!releaseMovement && !sneakRequested && ((Utils.isBindDown(mc.gameSettings.keyBindSprint) || ModuleManager.sprint.isEnabled()) && mc.thePlayer.movementInput.moveForward >= 0.8F && foodLvlMet && !mc.thePlayer.isSprinting()) && sprintingAllowed) {
            mc.thePlayer.setSprinting(true);
        }
        if (releaseMovement || sneakRequested || !sprintingAllowed) {
            mc.thePlayer.setSprinting(false);
        }
        if (!applyAllowSettings || allowRotating.isToggled()) {
    ...
    public void onJump(JumpEvent e) {
        if (e.entity == mc.thePlayer && shouldApplyAllowSettings() && !allowJumping.isToggled() && mc.currentScreen != null && !(mc.currentScreen instanceof ClickGui)) {
            e.setCanceled(true);
        }
    }
    ...
    public void onPrePlayerInput(PrePlayerInputEvent e) {
        ...
        boolean applyAllowSettings = shouldApplyAllowSettings();
        boolean sneak = (!applyAllowSettings || allowSneak.isToggled()) && e.isSneak();
        if (applyAllowSettings && !allowMidair.isToggled() && !mc.thePlayer.onGround) {
            e.setForward(0.0F);
            e.setStrafe(0.0F);
            e.setJump(false);
            sneak = false;
    ...
    private boolean shouldApplyAllowSettings() {
        return !(mc.currentScreen instanceof GuiInventory && shouldDelayOpenPacket() && delayedOpenPacket != null);
    }
    ...
    private boolean shouldReleaseMovement() {
        return (shouldApplyAllowSettings() && !allowMidair.isToggled() && !mc.thePlayer.onGround)
    ...
    private boolean isSneakRequested() {
        return (!shouldApplyAllowSettings() || allowSneak.isToggled())
    ...
    private void restoreMovementKeys() {
        ...
        boolean applyAllowSettings = shouldApplyAllowSettings();
        boolean sneak = (mc.currentScreen == null || !applyAllowSettings || allowSneak.isToggled()) && Utils.isBindDown(mc.gameSettings.keyBindSneak);
        KeyBinding.setKeyBindState(mc.gameSettings.keyBindSneak.getKeyCode(), sneak);
        boolean sprint = (mc.currentScreen == null || !applyAllowSettings || allowSprinting.isToggled() && !sneak) && Utils.isBindDown(mc.gameSettings.keyBindSprint);

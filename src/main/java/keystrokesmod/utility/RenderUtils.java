...
public class RenderUtils implements IMinecraftInstance {
    ...
    private static final int GUI_ITEM_LOGICAL_SIZE = 16;
    private static final ProjectionContext GUI_ITEM_PROJECTION = new ProjectionContext();
    private static final double[] GUI_ITEM_PROJECTED_POINT = new double[3];
    private static final IntBuffer GUI_ITEM_SCISSOR = BufferUtils.createIntBuffer(16);
    ...
    public static boolean projectTo2D(ProjectionContext context, double x, double y, double z, double[] output) {
        ...
        return projectTo2D(context, x, y, z, context.scaleFactor, output);
    }

    public static boolean projectTo2D(ProjectionContext context, double x, double y, double z, int scaleFactor, double[] output) {
        ...
        output[0] = context.screenCoords.get(0) / scaleFactor;
        output[1] = (Display.getHeight() - context.screenCoords.get(1)) / scaleFactor;
    ...
    public static void renderItemAndEffectIntoGui3D(ItemStack stack, int xPos, int yPos) {
        ...
        if (!renderGuiItemInScreenSpace(stack, xPos, yPos)) {
            renderGuiItemDirectFallback(stack, xPos, yPos);
        }
    }

    private static void renderGuiItemDirectFallback(ItemStack stack, int xPos, int yPos) {
        ...
        try {
            ...
            GlStateManager.pushMatrix();
            boolean normalizeEnabled = GL11.glIsEnabled(GL11.GL_NORMALIZE);
            try {
    ...
    private static boolean projectGuiItemPoint(double x, double y) {
        GUI_ITEM_PROJECTION.modelView.rewind();
        GUI_ITEM_PROJECTION.projection.rewind();
        GUI_ITEM_PROJECTION.viewport.rewind();
        GUI_ITEM_PROJECTION.screenCoords.clear();
        if (!GLU.gluProject(
                (float) x,
                (float) y,
                0.0F,
                GUI_ITEM_PROJECTION.modelView,
                GUI_ITEM_PROJECTION.projection,
                GUI_ITEM_PROJECTION.viewport,
                GUI_ITEM_PROJECTION.screenCoords
        )) {
            return false;
        }

        GUI_ITEM_PROJECTED_POINT[0] = GUI_ITEM_PROJECTION.screenCoords.get(0) - GUI_ITEM_PROJECTION.viewport.get(0);
        GUI_ITEM_PROJECTED_POINT[1] = GUI_ITEM_PROJECTION.viewport.get(3)
                - (GUI_ITEM_PROJECTION.screenCoords.get(1) - GUI_ITEM_PROJECTION.viewport.get(1));
        GUI_ITEM_PROJECTED_POINT[2] = GUI_ITEM_PROJECTION.screenCoords.get(2);
        return GUI_ITEM_PROJECTED_POINT[2] >= 0.0D && GUI_ITEM_PROJECTED_POINT[2] <= 1.0D;
    }

    private static boolean renderGuiItemInScreenSpace(ItemStack stack, int xPos, int yPos) {
        captureProjectionContext(GUI_ITEM_PROJECTION, 1);
        int viewportWidth = GUI_ITEM_PROJECTION.viewport.get(2);
        int viewportHeight = GUI_ITEM_PROJECTION.viewport.get(3);
        if (viewportWidth <= 0 || viewportHeight <= 0) {
            return false;
        }

        if (!projectGuiItemPoint(xPos + GUI_ITEM_LOGICAL_SIZE * 0.5D, yPos + GUI_ITEM_LOGICAL_SIZE * 0.5D)) {
            return false;
        }
        double centerX = GUI_ITEM_PROJECTED_POINT[0];
        double centerY = GUI_ITEM_PROJECTED_POINT[1];

        if (!projectGuiItemPoint(xPos + GUI_ITEM_LOGICAL_SIZE, yPos + GUI_ITEM_LOGICAL_SIZE * 0.5D)) {
            return false;
        }
        double halfWidth = Math.hypot(GUI_ITEM_PROJECTED_POINT[0] - centerX, GUI_ITEM_PROJECTED_POINT[1] - centerY);

        if (!projectGuiItemPoint(xPos + GUI_ITEM_LOGICAL_SIZE * 0.5D, yPos + GUI_ITEM_LOGICAL_SIZE)) {
            return false;
        }
        double halfHeight = Math.hypot(GUI_ITEM_PROJECTED_POINT[0] - centerX, GUI_ITEM_PROJECTED_POINT[1] - centerY);
        double renderScale = (halfWidth + halfHeight) / GUI_ITEM_LOGICAL_SIZE;
        if (!Double.isFinite(renderScale) || renderScale <= 0.0D) {
            return false;
        }

        // Inventory slots cover an exact number of physical pixels. Quantizing the projected
        // footprint avoids sampling the item model on fractional pixel boundaries.
        double renderPixelSize = Math.max(1.0D, Math.round(GUI_ITEM_LOGICAL_SIZE * renderScale));
        renderScale = renderPixelSize / GUI_ITEM_LOGICAL_SIZE;
        double screenX = Math.round(centerX - renderPixelSize * 0.5D);
        double screenY = Math.round(centerY - renderPixelSize * 0.5D);
        int previousMatrixMode = GL11.glGetInteger(GL11.GL_MATRIX_MODE);
        int previousProgram = OpenGlHelper.shadersSupported ? GL11.glGetInteger(35725) : 0;
        float previousBrightnessX = OpenGlHelper.lastBrightnessX;
        float previousBrightnessY = OpenGlHelper.lastBrightnessY;
        ...
        if (scissorEnabled) {
            GUI_ITEM_SCISSOR.clear();
            GL11.glGetInteger(GL11.GL_SCISSOR_BOX, GUI_ITEM_SCISSOR);
        }
        ...
        try {
            ...
            // Vanilla GUI scale is applied by the projection, not the item model-view matrix.
            GlStateManager.ortho(
                    0.0D,
                    viewportWidth / renderScale,
                    viewportHeight / renderScale,
                    0.0D,
                    1000.0D,
                    3000.0D
            );
            ...
            RenderHelper.enableGUIStandardItemLighting();
            OpenGlHelper.setLightmapTextureCoords(OpenGlHelper.lightmapTexUnit, 240.0F, 240.0F);
            GlStateManager.translate((float) (screenX / renderScale), (float) (screenY / renderScale), 0.0F);
            mc.getRenderItem().zLevel = 0.0F;
            ...
            if (scissorEnabled) {
                GL11.glEnable(GL11.GL_SCISSOR_TEST);
                GL11.glScissor(
                        GUI_ITEM_SCISSOR.get(0),
                        GUI_ITEM_SCISSOR.get(1),
                        GUI_ITEM_SCISSOR.get(2),
                        GUI_ITEM_SCISSOR.get(3)
                );
            }
            else {
                GL11.glDisable(GL11.GL_SCISSOR_TEST);
